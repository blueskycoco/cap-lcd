    ...
    ui(new Ui::MainWindow)
{
    ...
    //rdp decThread = new DecoderThread(this);
    // attache the sink and label UI widget to thread
    //rdp decThread->AttachWidgetSink(sink, ui->scanresult);
    //connect(decThread, &DecoderThread::resultReady, this, &handleResults);
    //rdp connect(decThread, &DecoderThread::finished, decThread, &QObject::deleteLater);
    //rdp decThread->start();
}
